...
\begin{algorithm}[H]
	...
	\Statex
		...
		\If{$(S_t, A_t) \notin \lbrace (S_0, A_0), (S_1, A_0), \ldots (S_{t-1}, A_{t-1}) \rbrace$}:\Comment{If first visit}
		\State \texttt{append} $G$ to $\texttt{returns}[S_t, A_t]$
		\Let{Q($S_{t}, A_t$)}{\texttt{average}(\texttt{returns}[$S_t, A_t$])}
...
\ix{Temporal-Difference Learning}\emph{Temporal-Difference Learning} (TD Learning) is a combination of Monte Carlo and Dynamic Programming ideas. Like MC methods, TD learning uses raw experience without an environmental model but it also bootstraps, using estimates of successor states to improve state estimates. 
...
\end{align}
This is known as the $TD(0)$ update, an estimate of the value of state as it samples the reward and uses an estimate of the successor state.\marginpar{\centering \includegraphics[width=0.2\marginparwidth]{figs/td_zero_backup}\\\small $TD(0)$ Backup Diagram} 
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		\Require{
			\textbf{Input:} \Statex \hspace{\algorithmicindent} policy $\pi$ to be evaluated. \Statex \hspace{\algorithmicindent} step size $\alpha \in [0, 1]$
		}\Statex 
		\Require{
			\textbf{Initialisation:} \Statex \hspace{\algorithmicindent} initialise $V(s)$ arbitarily for all $s \in \altmathcal{S}$ except that $V(\text{terminal})=0$. Initalise \
		} 
		\Statex
		\For{each episode}: 
		...
		\Let{$A$}{$ \pi(S) $} 
		\State \text{Perform action $A$, observe $R, S'$}
...
\ix{Advantages of TD Prediction}TD methods can be implemented in an online, fully incremental fashion. This is beneficial because many tasks have very long episodes which result in MC methods learning slowly. MC methods often have to neglect returns gained from actions which are inconsisent with the policy we are attemping to evaluate, but since TD learning uses \emph{transitions}, it is much less suspectible to this problem. 
...
\ix{Batch Updating}\emph{Batch updating} is when the update is computed across a batch of training episodes, the value function only being updated once for each batch. For small enough $\alpha$, batch $\alpha$-MC converges to the sample averages of actual returns. 

\vspace{1em}
\noindent\fbox{%
	\parbox{\textwidth}{%
		{\centering
			\textbf{Temporal-Difference vs Monte Carlo Prediction} \\
		}
		We are faced with a \emph{prediction problem} in an unknown Markov reward process without discounting. Suppose that we observe the following episodes. 
		\begin{align}
			&\texttt{A}, 0, \texttt{B}, 0 \quad && \texttt{B}, 0  \quad &&& \texttt{B}, 1 \quad &&&&\texttt{B}, 1  \nonumber \\
			&\texttt{B}, 1 \quad && \texttt{B}, 1  \quad &&& \texttt{B}, 1 \quad &&&&\texttt{B}, 1  \nonumber 
		\end{align}
		where, for example, the first episode indicates that we started in state \texttt{A}, transitioned to state \texttt{B} with reward $ 0 $ etc. It is clear that the value of state \texttt{B} is $0.75$, as this is the frequency that \texttt{B} returned reward $1$ before termination. What about state \texttt{A}?\\ \\
		TD$(0)$ would estimate of $V(\texttt{A})$ as $0.75$; it would observe that every state the MRP was in state \texttt{A}, it transitioned to \texttt{B} and the value of \texttt{B} is $0.75$. This can be understood as using the MRP assumption. This does not minimise error on the training episodes, but we might expect this to be produce lower error on \emph{future error}. In fact, this prediction is the value for the \emph{maximum-likelihood model of the MRP}, also known the \emph{certainty-equivalence estimate}, as it assumes that this model is the true model.  \\\\
		MC would instead note that every time the MRP was in state \texttt{A}, the return was $0$ and estimate $V(\texttt{A})$ as $0$. Note that this answer gives \emph{minimum squared error on the training error}. 
	}%
}

\vspace{1em}
The non-batch version of these algorithms can be understood to be moving in these general directions, whilst not getting fully there. Note that with state space $n= |\altmathcal{S}|$, the maximum-likelihood MRP requires up to $n^2$ memory to compute the transition probabilities, and then computing the value function requires an additional order $n^3$ steps (if done conventionally). It is striking that TD methods can approximate this solution in order $n$. 

\ix{Sarsa}To solve the control problem, we must estimate action-values. Instead of considering transitions from states to states, we consider transitions from state-action pairs to state-action pairs, which again gives a Markov chain with a reward process. 
\begin{align}
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
	    ...
	    \Statex 
		\Require{
			\textbf{Initialisation:} \Statex \hspace{\algorithmicindent} $Q(s, a)$ arbitarily for all $s \in \altmathcal{S}, a \in \altmathcal{A}$ except that $Q(\texttt{terminal}, \cdot) = 0$
		} 
		\Statex
...
\ix{Maximisation Bias}The control algorithms discussed involve maximisation to construct target policies; a maximisation over expected values is used implicitly as an estimate of the maximum value, leading to significant positive bias. One way to view this problem is that this occurs because the same samples have been used for \emph{both} determining the best action \emph{and} estimating its value. 

\ix{Double Learning}\emph{Double Learning} attempts to reduce maximisation bias by instead maintaining two sets of value functions; one used to estimate the value, and the other used to estimate the optimal action. At each time-step, we update one set of values only. The values updated are often chosen with a coin-toss, and the behavioural policy could for example by the $\epsilon$-greedy policy using the sum of the independent $Q$ values. 
...
\noindent\fbox{%
	\parbox{\textwidth}{%
		...
		In certain situations, initial parts of the environment dynamics are known, but not necessarily the full dynamics. This is common in games, such as chess or tic-tac-toe. In these situations, it is often beneficial to place value functions over \emph{afterstates} i.e., the state after the known part of the dynamics function is applied. This reduces the size of the problem, as many state-action pairs have the same afterstates, and thus ought to have the same \emph{afterstate value function}. \\\\
